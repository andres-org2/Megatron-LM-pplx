...
class TransformerConfig(ModelParallelConfig):
    ...
    moe_flex_dispatcher_backend: Literal["deepep", "hybridep", "pplx_garden"] = "deepep"
    """[Experimental] The backend to use for flex token dispatcher. The default is "deepep".
    Options are "deepep", "hybridep", and "pplx_garden". Currently only "hybridep" backend
    supports the MNNVL case."""

    moe_pplx_garden_nets_per_gpu: int = 1
    """Number of NICs per GPU to expose to pplx-garden."""
    ...
    def __post_init__(self):
        ...
        if self.moe_token_dispatcher_type == "flex":
            ...
            if self.moe_flex_dispatcher_backend == "pplx_garden":
                if self.moe_pplx_garden_nets_per_gpu <= 0:
                    raise ValueError(
                        "Flex token dispatcher with pplx_garden backend requires "
                        "moe_pplx_garden_nets_per_gpu > 0"
                    )
                if self.moe_pad_expert_input_to_capacity:
                    raise ValueError(
                        "Flex token dispatcher with pplx_garden backend does not support "
                        "moe_pad_expert_input_to_capacity"
                    )
                ...
                if self.fp8 or self.fp4:
                    raise ValueError(
                        "Flex token dispatcher with pplx_garden backend does not yet support "
                        "fp8/fp4 execution"
                    )
        ...
        if (
            ...
            and self.moe_flex_dispatcher_backend == "pplx_garden"
            and self.overlap_moe_expert_parallel_comm
        ):
            raise ValueError(
                "Flex token dispatcher with pplx_garden backend does not support "
